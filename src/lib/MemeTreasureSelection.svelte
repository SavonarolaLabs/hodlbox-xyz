...
<div class="flex items-center gap-4">
	<button
		disabled={$selected_treasure.id == 0}
		on:click={clickPreviousChest}
	...
	<img src={base + '/' + $selected_treasure.img} alt="" style="max-width:400px;" />
	<button
		disabled={$selected_treasure.id == $all_treasures.length - 1}
		on:click={clickNextChest}
		style="width:100px;height:100px;"
	>
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
			><path
				fill={$selected_treasure.id == $all_treasures.length - 1 ? '#CCC3' : '#CCCD'}
